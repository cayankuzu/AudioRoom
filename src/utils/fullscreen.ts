/**
 * Çapraz tarayıcı fullscreen wrapper.
 *
 * Chrome/Firefox/Edge: standart Fullscreen API.
 * Safari (macOS + iPad): webkit* prefix.
 * Safari (iPhone): API hiç yok. `isSupported()` false döner; scroll-trick
 * ile adres barı gizlemeye fallback yapılır (gameLoop içinde).
 */
...
/**
 * Fullscreen isteği. User-gesture içinde çağrılmalı (tap/click handler).
 * Başarısız olursa reject eder; arayan genellikle `.catch(noop)` ile yutar.
 */
...
/**
 * iOS Safari (iPhone) gibi Fullscreen API'si olmayan tarayıcılarda adres
 * barını gizleme trick'i. `scrollTo(0, 1)` setTimeout ile çağrıldığında
 * adres barı geri çekilir. Ayrıca `body` yüksekliği `100dvh` ile sabit
 * tutulur ki sayfa gerçekten scroll olmasın.
 */
