...
export function startExperience(container: HTMLElement): void {
  ...
  /**
   * Brand footer `bootstrap.ts`'de `document.body` seviyesinde mount
   * edilir — burada ikinci bir instance yaratmıyoruz.
   */
  ...
  const mobileControls = input.isTouch
    ? createMobileControls(container, input, {
        ...
        /**
         * Kök AudioRoom hub'ına dön (iç entry hub kaldırıldı).
         */
        onGoBack: () => {
          if (
            window.confirm(
