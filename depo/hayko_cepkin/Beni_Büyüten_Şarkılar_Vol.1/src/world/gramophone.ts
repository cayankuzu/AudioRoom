...
const GROUND_PADDING = 0.18;

/**
 * GRAMOFON — kor turuncu pirinç tonları, koyu ahşap kasa. "placed"/"carried"
 * state machine ile oyuncunun yanında taşınır veya yere bırakılır.
 */
export function createGramophone(
  ...
  /** HAYKO CEPKİN pirinç plaket. */
  const plaqueGeo = new THREE.PlaneGeometry(0.28, 0.07);
  ...
  plaque.position.set(0, 0.118, 0.301);
  root.add(plaque);
  ...
  const activeGlow = new THREE.PointLight("#ffb97a", 0, 2.6, 2);
  ...
  return {
    ...
    update(time, delta, pose, getHeightAt) {
      /**
       * Yerde duran gramofon, dalgalı/dalgalanan zemin yüzünden zaman
       * içinde batabilir. Her frame en yüksek civar zemin örneklemini
       * alıp Y'yi smooth lerple eşliyoruz.
       */
      if (state.state === "placed" && getHeightAt) {
        const px = root.position.x;
        const pz = root.position.z;
        const probe = 0.42;
        const hC = getHeightAt(px, pz);
        const hN = getHeightAt(px, pz - probe);
        const hS = getHeightAt(px, pz + probe);
        const hE = getHeightAt(px + probe, pz);
        const hW = getHeightAt(px - probe, pz);
        const groundTop = Math.max(hC, hN, hS, hE, hW);
        const targetY = groundTop + GROUND_PADDING;
        const k = 1 - Math.exp(-6.0 * delta);
...
function createPlaqueTexture(): THREE.CanvasTexture {
  ...
  if (ctx) {
    const grad = ctx.createLinearGradient(0, 0, 320, 80);
    grad.addColorStop(0, "#8c5a20");
    grad.addColorStop(0.5, "#f0c47a");
    grad.addColorStop(1, "#8c5a20");
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, 320, 80);
    ctx.fillStyle = "#2a1005";
    ctx.font = "700 28px 'Outfit', 'Helvetica Neue', Arial";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("HAYKO CEPKİN", 160, 32);
    ctx.font = "500 11px 'Outfit', Arial";
    ctx.fillText("BENİ BÜYÜTEN ŞARKILAR · VOL.1", 160, 58);
  }
